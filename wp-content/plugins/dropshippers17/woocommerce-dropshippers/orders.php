...
**/
	...
	if ( $query->have_posts() ) {
		while ( $query->have_posts() ) {
			...
			$order_total = 0;
			$order_dropshippers = get_post_meta(get_the_ID(), 'dropshippers', true);
			...
			if( (sizeof($real_products) > 0) && ($order->status == 'completed' || $order->status == 'processing') ){
				$order_count++;
				$table_string .= '<tr class="alternate">';
				$table_string .= '<td class="column-columnname"><strong>' . $order_number . '</strong></td>';
				$table_string .= '<td class="column-columnname"><strong>' . $order->order_date . '</strong></td>';
				...
				foreach ($real_products as $item) {
					...
					if($options['text_string'] == "Yes"){
						$table_string .= "<li>" . $my_item_post_title . ' ' . $my_meta . ' x' . $item['qty'] . ' ('. __('FULL PRICE:','woocommerce-dropshippers') .' <span class="artic-toberewritten">' . woocommerce_price((float) $item['line_subtotal']) .'</span><span class="artic-tobereconverted" style="display:none;">'.((float) $item['line_subtotal']).'</span> - '. __('MY EARNINGS:','woocommerce-dropshippers') .' <span class="artic-toberewritten">' . woocommerce_price((float) $drop_price*$item['qty']) .'</span><span class="artic-tobereconverted" style="display:none;">'.((float) $drop_price*$item['qty']).'</span>)';
					}
					else{
						$table_string .= "<li>" . $my_item_post_title . ' ' . $my_meta . ' x' . $item['qty'] . '('. __('MY EARNINGS:','woocommerce-dropshippers') .' <span class="artic-toberewritten">' . woocommerce_price((float) $drop_price*$item['qty']) .'</span><span class="artic-tobereconverted" style="display:none;">'.((float) $drop_price*$item['qty']).'</span>)';
					}
					/*if($item['variation_id'] != 0){
						$prodvar = new WC_Product_Variation($item['variation_id']);
						foreach ($prodvar->get_attributes() as $key => $value){
							$table_string .= '<br/>' . $value['name'] . ': ' . $item[$value['name']];
						}
					}*/
					$table_string .= '</li>' . "\n";
				...
				$table_string .= '</ul></td>';
				$table_string .= '<td class="column-columnname">' . $woocommerce->countries->countries[$order->shipping_country] .'<br/>'. $order->get_formatted_shipping_address() . '</td>';
				if($options['can_see_email'] == "Yes" || $options['can_see_phone'] == "Yes"){
					...
					if($options['can_see_email'] == "Yes"){
						$table_string .= $order->billing_email . '<br/>';
					}
					if($options['can_see_phone'] == "Yes"){
						$table_string .= 'Tel: '. $tel;
					}
					if($options['can_see_email'] == "Yes"){
						$table_string .= '<div class="row-actions"><span><a href="mailto:'.$order->billing_email.'">'. __('Send an Email','woocommerce-dropshippers') .'</a></span></div>';
					}
					$table_string .= '</td>';
				}
				$table_string .= '<td class="column-columnname">';
	        	if($options['text_string'] == "Yes"){
	        		$table_string .= __('FULL TOTAL:','woocommerce-dropshippers') .'<span class="artic-toberewritten">' . woocommerce_price((float) $order_total) . '</span><span class="artic-tobereconverted" style="display:none;">'.((float) $order_total).'</span><br>';
	        	}
	        	$table_string .= __('MY EARNINGS:','woocommerce-dropshippers') . '<span class="artic-toberewritten">' . woocommerce_price((float) $drop_total_price) . '</span><span class="artic-tobereconverted" style="display:none;">'.((float) $drop_total_price).'</span>';
	        	$table_string .= "</td>\n";
	        	...
	        	$table_string .= '</td><td class="column-columnname">' . __( $order->status, 'woocommerce' );
	        	if(isset($order_dropshippers[wp_get_current_user()->user_login])){
		        	if($order_dropshippers[wp_get_current_user()->user_login] == "Shipped"){
	...
	<form name="month" action="" method="GET"> 
		...
		<input class="button" type="submit" value="Filter orders" />
	</form>
	<hr/>
	<table class="wp-list-table widefat fixed posts" cellspacing="0">
    <thead>
